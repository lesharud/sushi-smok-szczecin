...
Текущая локальная версия использует реальный Guest Checkout. Перед её deployment нужны серверные `SUPABASE_URL` и `SUPABASE_SECRET_KEY`, миграция и seed из [BACKEND.md](BACKEND.md). Customer Auth удалён. Сведения ниже о demo и необязательном Supabase относятся к предыдущему deployment. Не публикуйте Stage 1 без проверки базы и отдельного разрешения.
